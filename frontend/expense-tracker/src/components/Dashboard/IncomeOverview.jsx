/**
 * IncomeOverview.jsx
 * Displays a bar chart showing income trends over time,
 * highlighting recent income activity and total earnings.
 */
 ...
 *
 * @param {Object} props
 * @param {Array} props.incomes - List of income transactions
 * @param {boolean} props.loading - Loading state for chart
 * @param {Function} props.onAddIncome - Callback to open add-income modal
 */
const IncomeOverview = ({ incomes, loading = false, onAddIncome }) => {
  ...
  /**
   * Prepare chart data, total income, and max value
   * Memoized for performance optimization
   */
  const { chartData, totalIncome, maxAmount } = useMemo(() => {
    // Group income amounts by date
    ...
    const total = incomeArray.reduce(
  ...
  /**
   * Handles add-income button click
   */
  ...
  // Ensure Y-axis has readable spacing
  const chartMax = Math.max(
    1000,
    Math.ceil((maxAmount || 0) / 500) * 500
  );
              ...
              >
                {chartData.map((entry, index) => (
                  <Cell
                    ...
                    fill={
                      index % 2 === 0
                        ? 'url(#incomeBarPrimary)'
                        : 'url(#incomeBarSecondary)'
                    }
                  />
                ))}
              </Bar>
